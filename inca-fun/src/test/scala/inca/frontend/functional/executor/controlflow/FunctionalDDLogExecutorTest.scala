...
class FunctionalDDLogExecutorTest extends AnyFunSuite:
  ...
  // Way too slow and produces the "wrong" result. Aka an intermediate result. This must be some bug in DDLog. The
  // generated program looks fine.
  /*test("Interval Analysis") {
    ...
    compiled.setPostProcessingPipeline(CompiledFunctionalUnit.ddlogPostProcessingPipeline)
