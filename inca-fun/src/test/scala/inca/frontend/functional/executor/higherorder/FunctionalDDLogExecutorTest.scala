...
class FunctionalDDLogExecutorTest extends AnyFunSuite:
  ...
  test("Transitive") {
    ...
    var query = Relation.from("Set$$TInt_TInt$$enum", Seq("$set", "$elem$0", "$elem$1"), Seq(Seq(setAdt, null, null)))
    ...
    query = Relation.from("Set$$TInt_TInt$$enum", Seq("$set", "$elem$0", "$elem$1"), Seq(Seq(setAdt, null, null)))
    res = loaded.engine.read(query).project(1)
