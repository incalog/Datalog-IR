...
trait Collect[R] {
  ...
  def collectType(t: Type): Seq[R] = t match {
    ...
    case TName(_) => Seq()
    case TApply(_, tys) => tys.flatMap(collectType)
  ...
  def collectExpression(exp: Expression): Seq[R] = exp match {
    ...
    case _: (IntLit | DoubleLit | StringLit | BoolLit) => Seq()
    ...
    case Call(fun, tyArgs, args) => collectExpression(fun) ++ tyArgs.flatMap(collectType) ++ args.flatMap(collectExpression)
    case Tuple(exps) =>
