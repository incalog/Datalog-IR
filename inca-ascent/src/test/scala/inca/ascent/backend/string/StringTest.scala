package inca.ascent.backend.string
...
class StringTest extends AnyFunSuite:
  ...
  def executeMain(compiled: CompiledUnit): inca.ir.execution.Relation =
    val engine = new inca.ascent.backend.Executor().instantiate(compiled)
  ...
  test("OrdinalNumber") {
    ...
    val compiled = compile(mod)
    val res = executeMain(compiled)
    assert(res.entries.head.asInstanceOf[Double].isValidInt)
  }
