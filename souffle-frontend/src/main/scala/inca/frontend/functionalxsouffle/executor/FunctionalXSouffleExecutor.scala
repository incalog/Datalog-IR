package inca.frontend.functionalxsouffle.executor
...
object FunctionalXSouffleExecutor {
  case class Loaded(engine: AdvancedViatraQueryEngine, feed: Database, compiled: CompiledFunctionalAndSouffleModule) {
    ...
    def souffleInput(rels: SouffleRelations): EditScript = {
      ...
      val edits = rels.flatMap { case (name, tuples) =>
        compiled.souffleSigs.find(_.name == name) match {
          case Some(sig) =>
            inputProcessor.compile(tuples, sig).edits
          case None => throw new IllegalArgumentException(s"Relation $name is not defined in the Soufflé programm")
        }
      }.toSeq
  ...
  def compileFunction(fun: String, souffle: String, options: FunctionalOptions = FunctionalOptions()): CompiledFunctionalAndSouffleModule = {
    Compiler.compileFunctionalAndSouffle(fun, souffle, options)
  }
  ...
  def loadFunction(fun: String, souffle: String, options: FunctionalOptions = FunctionalOptions()): Loaded =
    loadFunction(compileFunction(fun, souffle, options))
}
