...
class Parser(source: Source) {
  ...
  def AnalysisContent[_: P]: P[Syntax.SouffleContent] =
    P(
      ComponentInitialization | ComponentDefinition | TypeDeclaration |
  ...
  def ComponentDefinition[_: P]: P[Syntax.ComponentDefinition] =
    P(".comp" ~ identifier ~ "{" ~ AnalysisContent.rep ~ "}").mapWithLoc(
      Syntax.ComponentDefinition.tupled
    )
  ...
  def RuleSignature[_: P]: P[Syntax.RuleSignature] =
    P(
      ".decl" ~ identifier ~ "(" ~ RuleParameter.rep(1, sep = ",") ~ ")" ~ "output".!.?
    ).mapWithLoc { case (rule, params, output) =>
      Syntax.RuleSignature(rule, params, output.isDefined)
    }
  ...
  def RuleHead[_: P]: P[Syntax.RuleHead] =
    P(identifier ~ "(" ~ Expression.rep(min = 1, sep = ",") ~ ")").mapWithLoc(
      Syntax.RuleHead.tupled
    )
  ...
  def RuleApplication[_: P]: P[Syntax.RelationApplication] =
    P(
      "!".!.? ~ (identifier ~ ".").? ~ identifier ~ "(" ~ Expression.rep(min = 1, sep = ",") ~ ")"
    ).mapWithLoc { case (neg, comp, ruleName, args) =>
      Syntax.RelationApplication(neg.isDefined, comp, ruleName, args)
    }
  def Equality[_: P]: P[Syntax.Equality] =
    P(Expression ~ ("!=" | "=").! ~ Expression).mapWithLoc { case (left, compare, right) =>
      Syntax.Equality(left, compare == "!=", right)
    }
  ...
  def BuiltInFunctionCall[_: P]: P[Syntax.BuiltInFunctionCall] =
    P(BuiltInFunction ~ "(" ~ Expression.rep(min = 1, sep = ",") ~ ")").mapWithLoc(
      Syntax.BuiltInFunctionCall.tupled
    )

  def BuiltInFunction[_: P]: P[Syntax.BuiltInFunction] = CatBuiltInFunction
  def CatBuiltInFunction[_: P]: P[Syntax.CatBuiltInFunction.type] =
    P("cat").map(_ => Syntax.CatBuiltInFunction)
  ...
  def identifier[_: P]: P[Syntax.Name] =
    P(((letter | "_" | "?") ~~ (letter | digit | "_").repX).!.filter(_ != "_")).mapWithLoc(
      Syntax.Name.apply
    )
