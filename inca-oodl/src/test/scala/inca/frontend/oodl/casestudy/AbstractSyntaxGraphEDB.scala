...
class AbstractSyntaxGraphEDB extends AnyFunSuiteLike:
  ...
  val defList = ExtensionalRelation("_defList", Seq(Param("list", TAny)))
  val nils = ExtensionalRelation("_nil", Seq(Param("list", TAny)))
  val cons = ExtensionalRelation("_con", Seq(Param("list", TAny), Param("hd", TAny), Param("tail", TAny)))
  val defs = ExtensionalRelation("_def", Seq(Param("def", TAny), Param("name", TString), Param("exp", TAny)))
  val nums = ExtensionalRelation("_num", Seq(Param("exp", TAny), Param("value", TInt)))
  val vars = ExtensionalRelation("_var", Seq(Param("exp", TAny), Param("name", TString)))
  val adds = ExtensionalRelation("_add", Seq(Param("exp", TAny), Param("lhs", TAny), Param("rhs", TAny)))

  val edgesDefs = Relation("edgesDefs",
    Seq(
      Param("defs", TDemand(TAny)), // Deflist
      Param("from", TAny), // Def
      Param("to", TAny) // Def
    ),
    Seq(
  ...
  test("AbstractSyntaxGraph can be run") {
      val prog = GenerateProgram.prog(0, 4, 2)
