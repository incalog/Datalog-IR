...
class DataModel(
  ...
  def directSupertypes(ty: Type): Iterable[Type] = ty match {
    ...
    case NothingType =>
      throw new UnsupportedOperationException("The supertypes of NothingType are not enumerable")
  }
  def directSubtypes(ty: Type): Iterable[Type] = ty match {
    ...
    case AnyType =>
      throw new UnsupportedOperationException("The subtypes of AnyType are not enumerable")
...
object DataModel {
  ...
  def from(nodes: NodeMetaInfo*): DataModel = {
    ...
    val links: Map[Link, Type] = Map.from(
      nodes.flatMap(n =>
        n.links.map { case (NamedLink(name), ty) =>
          (n.sort.name, name) -> ty
        })
    )
    val litLinks: Map[Link, LitType] = Map.from(
      nodes.flatMap(n =>
        n.litLinks.map { case (NamedLink(name), ty) =>
          (n.sort.name, name) -> ty
        })
    )
