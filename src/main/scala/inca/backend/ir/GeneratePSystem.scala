...
object GeneratePSystem {
  ...
  def compileModule(module: Module)(implicit env: RuleEnvironment): Source = {
    ...
    val myenv =
      env ++ module.pats.map(p =>
        p.name -> module.name) // makes sure this module's names are found first
    ...
    source"""
      ...
      object $name extends ${Init(tyPSystemModule, Term.Name(tyPSystemModule.toString), List())} {
        ...
        override val patterns: $tMap[String, () => $tyQuerySpecification] = $oMap(..${module.pats.filter(
        !_.isEmpty
      ).map(p => q"${p.name} -> (() => ${Term.Name(p.name)}.instance)").toList})
  ...
  private def compileAtom(atom: Atom)(implicit env: RuleEnvironment): Seq[Stat] = atom match {
    case Undef(_) =>
      throw new IllegalStateException(
        s"Cannot compile undef constraint. Use undef elimination transformation first."
      )
    ...
    case Call(name, args, transitive, neg) =>
      ...
      val callQuery =
        q"${Term.Name(module)}.${Term.Name(name)}.instance.getInternalQueryRepresentation"
    ...
    case Path(src, _, link, trg, trgTy) =>
      ...
      Seq(
        q"new TypeConstraint(body, Tuples.staticArityFlatTupleOf(${compileTerm(src)}, ${compileTerm(
            trg)}), $key)"
      )
  ...
  private def genLinkKey(link: Link, targetType: Datalog.Type): meta.Term = link match {
    ...
    case Datalog.NamedLink(TNode(name), field) =>
      targetType match {
        ...
        case _ =>
          throw new IllegalStateException(
            s"Generating LinkKey for NamedLink with target type $targetType not supported"
          )
      }
  ...
  private def compileComputation(
      lhs: Datalog.Term,
      computation: Computation
    )(implicit env: RuleEnvironment
    ): Seq[Stat] = computation match {
    ...
    case CustomAggregation(typ, _, agg, patName, args, aggregatedColumn) =>
      ...
      Seq(
        q"new $tAggregatorConstraint($boundAggOp, body, $argTuple, $callQuery, $result, $aggregatedColumn)"
      )
  }
