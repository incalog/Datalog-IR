...
object Desugaring extends Desugarable {
  ...
  override def trans(): DesugarTrans = new DesugarTrans {
    ...
    override def desugarStm(stm: Statement)(implicit gensym: Gensym): Seq[Statement] = stm match {
      case Match(matchee, cases) =>
        ...
        val bodies = cases.flatMap { cas =>
          ...
          notPatsAlternatives = for {
            notPatsAlt <- notPatsAlternatives
    ...
    def desugarPat(exp: Expression, pat: Pattern)(implicit gensym: Gensym): Seq[Statement] =
      pat match {
        case NodePattern(c, bindings) =>
          ...
          bindings.foreach { case binding @ PatternBinding(field, subpat) =>
            val typ = binding.typ.getOrElse(
              throw new IllegalArgumentException(s"Cannot desugar untyped pattern binding $binding")
            )
        ...
        case ScalaPattern(Eval(Scala(fun)), noArgs, args) =>
          ...
          if (noArgs) {
            result += Assert(
              Eval(Seq(EvalParam(matchee)), Scala(q"$matcheeTerm == $fun")).typed(TScalaBoolean)
            )
          } else {
            val vars = args.map(_ => meta.Term.Name(gensym.fresh("scalaPatArg"))).toList
            val matchCode = meta.Term.Match(
              matcheeTerm,
              List(
    ...
    def desugarNegatedPat(
        exp: Expression,
        pat: Pattern
      )(implicit gensym: Gensym
      ): Seq[Seq[Statement]] = pat match {
      case NodePattern(c, bindings) =>
        ...
        val alts = bindings.flatMap { case binding @ PatternBinding(field, subpat) =>
          ...
          val patAlts =
            desugarNegatedPat(PathAccess(matcheeCasted, NamedLink(field)).typed(typ), subpat)
      ...
      case ScalaPattern(Eval(Scala(fun)), noArgs, args) =>
        ...
        import meta.quasiquotes._
        if (noArgs) {
          val mismatch = ensureVar :+ Assert(
            Eval(Seq(EvalParam(matchee)), Scala(q"$matcheeTerm != $fun")).typed(TScalaBoolean)
          )
          Seq(mismatch)
        } else {
          val vars = args.map(_ => meta.Term.Name(gensym.fresh("scalaPatArg"))).toList
          val mistmatchCode = meta.Term.Match(
            matcheeTerm,
            List(
          ...
          val mismatch = ensureVar :+ Assert(
            Eval(Seq(EvalParam(matchee)), Scala(mistmatchCode)).typed(TScalaBoolean)
          )
