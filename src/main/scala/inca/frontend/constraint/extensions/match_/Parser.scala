...
import inca.frontend.util.ParserUtils.nl_!
import inca.frontend.util.ParserUtils.sp
...
trait Parser extends CoreParser {
  ...
  protected[frontend] def matchStatement[_: P]: P[Statement] =
    P(exp ~ "match" ~ "{" ~ P(sp ~~ case_ ~~ sp).repX(sep = nl_!) ~ "}").mapWithLoc {
      case (e, cs) => Match(e, cs)
    }
  ...
  protected[frontend] def pattern[_: P]: P[Pattern] =
    P(
      tuplePattern | namedPattern | nodePattern | wildcardPattern | varPattern
        | literalPattern | scalaPattern
    )

  protected[frontend] def patternBinding[_: P]: P[PatternBinding] =
    P(identifier ~ "=" ~ pattern).mapWithLoc { case (name, pattern) =>
      PatternBinding(name, pattern)
    } |
      P(identifier).mapWithLoc(name => PatternBinding(name, VarPattern(name)))

  protected[frontend] def nodePattern[_: P]: P[Pattern] =
    P(tNode ~ "(" ~ P(patternBinding).rep(sep = ",") ~ ")").mapWithLoc { case (node, bindings) =>
      NodePattern(node, bindings)
    }
