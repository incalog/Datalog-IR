  ...
  override def trans(): DesugarTrans = new DesugarTrans {
    ...
    override def desugarStm(stm: Statement)(implicit gensym: Gensym): Seq[Statement] = stm match {
      case Forall(name, exp, body) =>
        exp.typ.getOrElse(
          throw new IllegalArgumentException(s"Cannot support forall over untyped expression $exp")
        ) match {
          case TList(ty) =>
          ...
          case ty =>
            throw new IllegalArgumentException(
              s"Forall loop expression $exp must have iterable type, but was type $ty"
            )
        }

      case Exists(name, exp, body) =>
        exp.typ.getOrElse(
          throw new IllegalArgumentException(s"Cannot support exists over untyped expression $exp")
        ) match {
          case TList(ty) =>
    ...
    private def makeCondFun(
      ...
      ): Seq[(Name, Option[Type])] = {
      val newbody = Body(
        Seq(
          Foreach(name, exp, Body(body.stmts.flatMap(desugarStm) :+ Yield(Var(name))))
        )
      )
