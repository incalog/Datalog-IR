...
object CollectFreeScalaVars {
  ...
  /**
   * Computes the set of free(unbound) variables in this AST. Important note: using variables with
   * backtickets in patterns does not work properly with the computation of free vars. The variable
   * will be counted as bound even if it's meant to reference an undefined constant
   *
   * @param term
   *   the AST
   * @param bound
   *   an optional initial set of bound variables
   * @return
  ...
  /**
   * Loads the free variables of the whole AST and the variables bound at the level of the node into
   * the scope
   *
   * @param term
   *   the AST
   * @param scope
   *   the scope of the AST node into which the variables are loaded
   */
  ...
  private def freeVars(decl: Decl, scope: Scope): Unit = throw new IllegalArgumentException(
    "Decls are not supported"
  )
  ...
  /**
   * A scope represents the free and bound variables in a specific code block. Blocks for example
   * create a new scope nested in the current one so variables newly bound in this nested scope are
   * only bound there and not in it's parent scope. For the purpose of finding all free variables in
   * an given AST this implementation uses only one set of free vars. This means once an initial
   * scope for the root of the tree is created, it and all it's nested scopes share this set while
   * the set of bound variables is cloned each time a new nested scope is created
   *
   * @param free
   *   the set of free variables known in this scope
   * @param bound
   *   the set of bound variables known in this scope
   */
  private class Scope(val free: mutable.Set[Name], val bound: mutable.Set[Name]) {
    ...
    /**
     * Adds the name to the set of known free variables. Does nothing if the name is already
     * referencing a known free variable
     *
     * @param name
     *   the name of the new free variable
     */
    ...
    /**
     * Adds the name to the set of known bound variables. Does nothing if the name is already
     * referencing a known bound variable
     *
     * @param name
     *   the name of the new bound variable
     */
