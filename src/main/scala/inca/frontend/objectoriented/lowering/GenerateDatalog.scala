...
class GenerateDatalog(typedModule: Module, coreModule: Module) {
  ...
  /*def loadSection3Example(): Datalog.Module = {
    val Module(name, imports, classes) = coreModule

    val thisParam = Datalog.Param("this", GP_URI)
    val thisVar = Datalog.Var(thisParam.name)

    def generateClassPattern(className: String, fieldClassName: String, params: Seq[Datalog.Param]): Seq[Datalog.Pattern] = {
      val allocIn = Datalog.Param("allocIn", Datalog.TScalaInt)
      val allocOut = Datalog.Param("allocOut", Datalog.TScalaInt)

      val allocInVar = Datalog.Var(allocIn.name)
      val allocOutVar = Datalog.Var(allocOut.name)

      val constrPattern = Datalog.Pattern(None, "constr$"+className, thisParam +: params :+ allocIn :+ allocOut, Seq(
        Datalog.Body(
          Datalog.Computed(thisVar, Datalog.Evaluation(
            Seq(allocInVar -> allocIn.typ),
            thisParam.typ,
            Scala(q"(allocIn: Int) => inca.runtime.data.ObjectID($className, allocIn)")
          ))
            +: Datalog.Call(className, Seq(thisVar))
            +: params.map { p =>
            Datalog.Call(fieldClassName + "$" + p.name, Seq(thisVar, Datalog.Var(p.name)))
          }
            :+ Datalog.Computed(allocOutVar, Datalog.Evaluation(
              Seq(allocInVar -> allocIn.typ),
              allocOut.typ,
              Scala(q"(allocIn: Int) => allocIn + 1")
            ))
        ))
      )
      val classPattern = Datalog.Pattern(None, className, Seq(thisParam), Seq())

      val fieldPattern =
        if (fieldClassName == className) {
          params.map { p =>
            Datalog.Pattern(None, className + "$" + p.name, Seq(thisParam, p), Seq())
          }
        } else {
          Seq()
        }
      classPattern +: constrPattern +: fieldPattern
    }

    val clsPat1 :: constrPat1 :: fieldPats1 = generateClassPattern("Expr", "Expr", Seq())
    val clsPat2 :: constrPat2 :: fieldPats2 = generateClassPattern("Num", "Num", Seq(Datalog.Param("value", Datalog.TScalaInt)))
    val clsPat3 :: constrPat3 :: fieldPats3 = generateClassPattern("BinOp", "BinOp", Seq(Datalog.Param("lhs", GP_URI), Datalog.Param("rhs", GP_URI)))
    val clsPat4 :: constrPat4 :: fieldPats4 = generateClassPattern("AddOp", "BinOp", Seq(Datalog.Param("lhs", GP_URI), Datalog.Param("rhs", GP_URI)))

    val outParam = Datalog.Param("out", Datalog.TScalaInt)
    val outVar = Datalog.Var(outParam.name)
    val evalExpr = Datalog.Pattern(None, "Expr$eval", Seq(thisParam, outParam), Seq(Datalog.Body(Seq(
      Datalog.Eq(outVar, Datalog.IntConstant(0))
    ))))
    val evalNum = Datalog.Pattern(None, "Num$eval", Seq(thisParam, outParam), Seq(Datalog.Body(Seq(
      Datalog.Call("Num$value", Seq(thisVar, outVar))
        .addHint(MagicSetHints.IgnoreCall)
        .addHint(MagicSetHints.FixedAdornment(Seq(true, true)))
    ))))

    val lhsVar = Datalog.Var("lhs")
    val rhsVar = Datalog.Var("rhs")
    val lhsOutVar = Datalog.Var("out1")
    val rhsOutVar = Datalog.Var("out2")

    val evalAddOp = Datalog.Pattern(None, "AddOp$eval", Seq(thisParam, outParam), Seq(Datalog.Body(Seq(
      Datalog.Call("BinOp$lhs", Seq(thisVar, lhsVar))
        .addHint(MagicSetHints.IgnoreCall)
        .addHint(MagicSetHints.FixedAdornment(Seq(true, true))),
      Datalog.Call("BinOp$rhs", Seq(thisVar, rhsVar))
      .addHint(MagicSetHints.IgnoreCall)
      .addHint(MagicSetHints.FixedAdornment(Seq(true, true))),
      Datalog.Call("dispatch$eval", Seq(lhsVar, lhsOutVar)),
      Datalog.Call("dispatch$eval", Seq(rhsVar, rhsOutVar)),
      Datalog.Computed(outVar, Datalog.Evaluation(
        Seq(lhsOutVar -> outParam.typ, rhsOutVar -> outParam.typ),
        outParam.typ,
        Scala(q"(lhsIn: Int, rhsIn: Int) => lhsIn + rhsIn")
      ))
    ))))

    val dispatchPat = Datalog.Pattern(None, "dispatch$eval", Seq(thisParam, outParam), Seq(
      Datalog.Body(Seq(
        Datalog.Call("Expr", Seq(thisVar))
          .addHint(MagicSetHints.IgnoreCall)
          .addHint(MagicSetHints.FixedAdornment(Seq(true)))
        , Datalog.Call("Expr$eval", Seq(thisVar, outVar))
      )),
      Datalog.Body(Seq(
        Datalog.Call("Num", Seq(thisVar))
          .addHint(MagicSetHints.IgnoreCall)
          .addHint(MagicSetHints.FixedAdornment(Seq(true)))
        , Datalog.Call("Num$eval", Seq(thisVar, outVar))
      )),
      Datalog.Body(Seq(
        Datalog.Call("BinOp", Seq(thisVar))
          .addHint(MagicSetHints.IgnoreCall)
          .addHint(MagicSetHints.FixedAdornment(Seq(true)))
        , Datalog.Call("Expr$eval", Seq(thisVar, outVar))
      )),
      Datalog.Body(Seq(
        Datalog.Call("AddOp", Seq(thisVar))
          .addHint(MagicSetHints.IgnoreCall)
          .addHint(MagicSetHints.FixedAdornment(Seq(true)))
        , Datalog.Call("AddOp$eval", Seq(thisVar, outVar))
      ))
    ))

    val mainPat = Datalog.Pattern(None, "AddOp$main", Seq(outParam), Seq(
      Datalog.Body(Seq(
        Datalog.Call("constr$Num", Seq(Datalog.Var("n1"), Datalog.IntConstant(5), Datalog.IntConstant(0), Datalog.Var("allocOut$0"))),
        Datalog.Call("constr$Num", Seq(Datalog.Var("n2"), Datalog.IntConstant(5), Datalog.Var("allocOut$0"), Datalog.Var("allocOut$1"))),
        Datalog.Call("constr$AddOp", Seq(Datalog.Var("addOp"), Datalog.Var("n1"), Datalog.Var("n2"), Datalog.Var("allocOut$1"), Datalog.Var("_"))),
        Datalog.Call("dispatch$eval", Seq(Datalog.Var("addOp"), outVar))
      ))
    )).addHint(MagicSetHints.Main(Seq(false)))

    generatedPatterns ++= Seq(clsPat1, clsPat2, clsPat3, clsPat4)
    generatedPatterns ++= Seq(constrPat1, constrPat2, constrPat3, constrPat4)
    generatedPatterns ++= fieldPats1 ++ fieldPats2 ++ fieldPats3 ++ fieldPats4
    generatedPatterns ++= Seq(evalExpr, evalNum, evalAddOp)
    generatedPatterns += dispatchPat
    generatedPatterns += mainPat
