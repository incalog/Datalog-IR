...
case class ClassDef(annos: Seq[Annotation], vis: Option[Visibility], name: Name, genericTypeParams: Option[Seq[Name]], parentClassRefs: Seq[ClassRef], content: Seq[ClassContent])
  ...
  def isGeneric: Boolean = genericTypeParams.isDefined
  ...
  def prettyprint(implicit indent: String): String = {
    ...
    val parentClassesS = if (parentClassRefs.nonEmpty) {
      ...
      s""
    val genericTypeParam = if (isGeneric) s"[${this.genericTypeParams.get}]" else ""
...
case class MethodDef(annos: Seq[Annotation], vis: Option[Visibility], name: Name, genericTypeParams: Option[Seq[Name]], params: Seq[Param], outType: Type, body: Seq[Statement])
