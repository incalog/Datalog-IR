...
class GenerateScala {
  ...
  private val oMockURI = symbolOf(MockURI)
  ...
  def genDataDef(data: DataDef): Unit = createIfNeeded(data) {
    ...
    val constrs = data.constrs.map { case DataConstructor(syntax.Name(name), params) =>
      ...
      q"""case class ${MetaType.Name(
          name)}(..$scalaParams) extends {} with $dataTyp() with truediff.GenericDiffable() { this =>
              this.withURI($oMockURI($name, Seq(..$terms)))
