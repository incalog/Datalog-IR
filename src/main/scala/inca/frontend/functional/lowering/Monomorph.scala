...
import inca.frontend.functional.util.{Collect, TypeUtil}
...
class Monomorph(module: Module) {
  ...
  def transModule(): Module = {
    ...
    Module(module.name, module.imports, transformedMonomorphicContent ++ monomorphicDataDefs.values ++ monomorphicFunctionDefs.values)
  }
  ...
  private def collectTypeApplications(): Seq[(Name, Seq[Type])] = {
    val collector = new Collect[(Name, Seq[Type])] {
      ...
      override def collectExpression(exp: Expression): Seq[(Name, Seq[Type])] = exp match {
        case Call(v@Var(name), tyArgs, args, _) if tyArgs.nonEmpty && tyArgs.forall(isMonomorphic) =>
  ...
  private def monomorph(ty: Type)(implicit subst: Map[TName, Type]): Type = ty match {
    ...
    case TConstr(name, tys) => TName(polymorphicToMonomorphic(name, tys.map(monomorph)))
    case TScala(ty) => TScala(ty)
    case TOption(ty) => TOption(monomorph(ty))
  ...
  private def monomorph(exp: Expression)(implicit subst: Map[TName, Type]): Expression = exp match {
    ...
    case Call(fun, tyArgs, args, trans) => Call(monomorph(fun), tyArgs, args.map(monomorph), trans)
    ...
    case BaseLit(code) => BaseLit(code)
    case BaseApply(fun, args) => BaseApply(fun, args.map(monomorph))
    case BaseApplyInfix(left, op, right) => BaseApplyInfix(monomorph(left), op, monomorph(right))
    case BaseApplyMethod(recv, method, args) =>
      val monoArgs = args match {
        case Some(value) => Some(value.map(monomorph))
        case None => None
      }
      BaseApplyMethod(monomorph(recv), method, monoArgs)
    case BaseApplyUnary(op, e) => BaseApplyUnary(op, monomorph(e))
    case TypeCast(exp, ty) => TypeCast(monomorph(exp), ty)
    case NoneExp() => NoneExp()
    case SomeExp(e) => SomeExp(monomorph(e))
  }
