...
class Defunctionalize(module: Module) {
        ...
        val data = DataDef(
          ...
          funs.map { case AnonFun(_, _, _, defunName, freevars) =>
            val constr = DataConstructor(
              Name(defunName),
              freevars.map(v => transformType(v.typ.getOrElse(TAny)))
            )
            newVarTargets += Name(defunName) -> (
              (
                constr,
                constr.constructorType(Name(funData(tfun)))
              )
            )
            constr
          }
  ...
  def transformExp(exp: Expression): Expression = exp match {
    ...
    case Let(names, anno, bound, body) =>
      Let(names, anno.map(transformType), transformExp(bound), transformExp(body)).sourceLocFrom(
        exp
      )
    case If(cnd, thn, els) =>
    ...
    case Call(fun, tyArgs, args, transitive) =>
      ...
      fun match {
        case v @ Var(name)
            if v.target.forall(_.isInstanceOf[FunctionDef]) || v.target.forall(
        ...
        case _ =>
          val tfun @ TFun(_, _) = fun.typ.getOrElse(
            throw new IllegalStateException(
              s"Expression at function position of call has to be of function type"
            )
          ).asInstanceOf[TFun]
          // call defun apply
          Call(
    ...
    case lam: Lambda =>
      ...
      val tfun @ TFun(_, _) = lam.typ.getOrElse(
        throw new IllegalStateException(s"Lambda has to be of function type")
      ).asInstanceOf[TFun]
