...
case class Let(names: Seq[Name], anno: Option[Type], bound: Expression, body: Expression)
  ...
  override def freeTvars: Seq[TData] =
...
case class Call(fun: Expression, args: Seq[Expression], transitive: Boolean = false)
    extends Expression {
...
case class ConstructorPattern(constr: Name, args: Seq[Name])
    extends Pattern
