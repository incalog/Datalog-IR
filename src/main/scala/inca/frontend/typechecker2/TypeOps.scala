...
import truechange.{AnyType, JavaLitType, ListType, NothingType, OptionType, SortType}
...
//    case (_, TUnbounded(wrappedTy)) if ty1 == wrappedTy => Some(wrappedTy)
  ...
  def truechangeTypeToTypeAnno(ty: truechange.Type): TypeAnno = ty match {
    ...
    case _ => throw new IllegalArgumentException(s"TODO conversion of $ty not supported")
  }
