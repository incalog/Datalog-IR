...
@diffableType sealed trait stmt
object stmt{
...
@diffableType sealed trait expr
object expr{
...
@diffableType sealed trait expr_context
object expr_context{
...
@diffableType sealed trait slice
object slice{
...
@diffableType sealed trait boolop
object boolop{
  case object And extends boolop
  case object Or extends boolop
}

@diffableType sealed trait operator
...
@diffableType sealed trait unaryop
object unaryop{
...
@diffableType sealed trait cmpop
object cmpop{
...
@diffableType case class comprehension(target: expr, iter: expr, ifs: Seq[expr])

// not sure what to call the first argument for raise and except
@diffableType sealed trait excepthandler
object excepthandler{
  case class ExceptHandler(`type`: Option[expr], name: Option[expr], body: Seq[stmt]) extends excepthandler
}

@diffableType case class arguments(args: Seq[expr], vararg: Option[identifier], kwarg: Option[identifier], defaults: Seq[expr])

// keyword arguments supplied to call
@diffableType case class keyword(arg: identifier, value: expr)

// import name with optional 'as' alias.
@diffableType case class alias(name: identifier, asname: Option[identifier])