...
class HigherOrderTest extends AnyFunSuite {
  ...
  test("applyFun") {
    ...
    assert(fun.execute("main_f", Seq()) == fun.result(q"6"))
    fun.printAllMatches()
  }

  test("lambda") {
    val fun = loadFunction(HigherOrder.lambda)
    assert(fun.execute("main_f", Seq()) == fun.result(q"21"))
    fun.printAllMatches()
  }

  test("lambdaHigherOrder") {
    val fun = loadFunction(HigherOrder.lambdaHigherOrder)
    assert(fun.execute("main_f", Seq()) == fun.result(q"63"))
    fun.printAllMatches()
  }

  test("composeFun") {
    val fun = loadFunction(HigherOrder.composeFun)
    assert(fun.execute("main_f", Seq()) == fun.result(meta.Lit.String(Math.sqrt(2).toString)))
    fun.printAllMatches()
  }
