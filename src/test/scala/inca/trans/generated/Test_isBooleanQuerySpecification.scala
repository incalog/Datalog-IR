...
import inca.analyzedLangs.expLang.BooleanLit
...
object Test_isBooleanQuerySpecification {
  ...
  private final object GeneratedPQuery extends BasePQuery(PVisibility.PUBLIC) {
    ...
    private val param_in: PParameter = new PParameter("in", "inca.analyzedLangs.expLang.BooleanLit", new TFInputKey.NodeTypeKey(MetaElements.NodeType(classOf[BooleanLit])))
    ...
    override protected def doGetContainedBodies(): util.Set[PBody] = {
      val bodies: util.Set[PBody] = util.Set.of {
        ...
        new TypeConstraint(body, Tuples.flatTupleOf(var_in), new TFInputKey.NodeTypeKey(MetaElements.NodeType(classOf[BooleanLit])))
