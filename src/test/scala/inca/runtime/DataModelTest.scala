...
class DataModelTest extends AnyFunSuite {
  ...
  test("0 step trans closure") {
    val metaInfo = new DataModel(Set(), MultiDict(add -> exp, mult -> exp), null, null)
  ...
  test("1 step trans closure") {
    val metaInfo =
      new DataModel(Set(), MultiDict(add -> iNumExp, mult -> iNumExp, iNumExp -> exp), null, null)
    metaInfo.nodeSupertypes.toSet should contain theSameElementsAs Set(
      add -> iNumExp,
      add -> exp,
      mult -> iNumExp,
      mult -> exp,
    ...
    metaInfo.directNodeSubtypes.toSet should contain theSameElementsAs Set(
      exp -> iNumExp,
      iNumExp -> add,
      iNumExp -> mult
    )
    metaInfo.nodeSubtypes.toSet should contain theSameElementsAs Set(
      exp -> add,
      exp -> mult,
      exp -> iNumExp,
      iNumExp -> add,
  ...
  test("2 step trans closure") {
    val metaInfo = new DataModel(
      Set(),
      MultiDict(add -> iNumExp, mult -> iNumExp, iNumExp -> exp, exp -> node),
      null,
      null
    )
    metaInfo.nodeSupertypes.toSet should contain theSameElementsAs Set(
