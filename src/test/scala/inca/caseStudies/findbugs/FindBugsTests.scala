...
class FindBugsTests extends AnyFunSuite {
  ...
  test("Confused Inheritance") {
    ...
    val spec =
      Compiler.compileConstraint(code, options).psystemModule.patterns("confusedInheritance")

    val feed = EnginePool.loadDatabase(scope, TimelyReteBackendFactory.FIRST_ONLY_SEQUENTIAL)
    val matcher =
      EnginePool.loadQuery(spec(), scope, TimelyReteBackendFactory.FIRST_ONLY_SEQUENTIAL)
    ...
    val clazz = ClassDeclaration(
      ...
      List(
        ...
        FieldDeclaration("bar", ProtectedVisibility())
      )
    )
