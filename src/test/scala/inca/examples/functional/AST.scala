...
import inca.examples.functional.ADT._
...
object AST {
  ...
  val baseExample2: Module = module(
    FunctionDef(
      Seq(MainFunctionAnno),
      None,
      Name("main"),
    ...
    FunctionDef(
      ...
      Let(
        ...
        Let(
          Seq(Name("y")),
          None,
          BaseLit(q"3", TScalaInt),
          BaseApplyInfix(Var("x"), "+", BaseApplyInfix(BaseLit(q"12", TScalaInt), "*", Var("y")))
        )
      )
    )
  )

  val ifExample: Module = module(
    FunctionDef(
      Seq(MainFunctionAnno),
      None,
      Name("main"),
      ...
      Let(
        ...
        Let(
          ...
          BaseApplyInfix(
            ...
            "+",
            If(
              BaseApplyInfix(Var("y"), ">", BaseLit(q"0", TScalaInt)),
              Var("y"),
  ...
  val incFun: FunctionDef = FunctionDef(
    Seq(),
    None,
    Name("inc"),
    Seq(Param(Name("n"), TScalaInt)),
    TScalaInt,
    BaseApplyInfix(Var("n"), "+", BaseLit(q"1", TScalaInt))
  )
  val incMain: FunctionDef = FunctionDef(
    Seq(MainFunctionAnno),
    None,
    Name("main"),
    Seq(),
  ...
  val factFun: FunctionDef = FunctionDef(
    Seq(),
    None,
    Name("fact"),
    Seq(Param(Name("n"), TScalaInt)),
  ...
  val factMain: FunctionDef = FunctionDef(
    Seq(MainFunctionAnno),
    None,
    Name("main"),
    Seq(Param(Name("n"), TScalaInt)),
  ...
  val plusFun: FunctionDef = FunctionDef(
    Seq(),
    None,
    Name("plus"),
    Seq(Param(Name("m"), TNat), Param(Name("n"), TNat)),
    TNat,
    Match(
      Var(Name("m")),
      Seq(
        ...
        ConstructorPattern(Name("Succ"), Seq(Name("pred"))) ->
          Call(
            Var(Name("Succ")),
            Seq(Call(Var(Name("plus")), Seq(Var(Name("pred")), Var(Name("n")))))
          )
      )
    )
  )
  val plusMain: FunctionDef = FunctionDef(
    Seq(MainFunctionAnno),
    None,
    Name("main"),
    Seq(),
      ...
      Seq(
        Call(
          ...
          Seq(
            Call(
              Var(Name("Succ")),
              Seq(Call(Var(Name("Succ")), Seq(Call(Var(Name("Zero")), Seq()))))
            )
          )
        ),
  ...
  val plusRealMain: FunctionDef = FunctionDef(
    Seq(MainFunctionAnno),
    None,
    Name("main"),
    Seq(Param(Name("x"), TNat), Param(Name("y"), TNat)),
    TNat,
