...
class TestEnum extends AnyFlatSpec with IncaMatchers {
  ...
  val options: CompilerOptions = CompilerOptions(scope.langMetaInfo, frontend = new EnumFrontend {})
  ...
  "desugaring" should "eliminate foreach enum" in {
    ...
    assertDesugar(core, sugared, CompilerOptions(scope.langMetaInfo, frontend = new EnumFrontend with ForeachFrontend with IfThenElseFrontend {}))
  }
