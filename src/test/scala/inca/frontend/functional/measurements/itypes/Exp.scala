...
object Exp {
  def fold[A](
    ...
    case Add(l, r) =>
      add(
        fold(num, add, app, lam, vari, let, l),
        fold(num, add, app, lam , vari, let, r))
    case App(l, r) =>
      app(
        fold(num, add, app, lam, vari, let, l),
        fold(num, add, app, lam, vari, let, r))
    ...
    case Let(n, bound, body) =>
      let(
        n,
        fold(num, add, app, lam, vari, let, bound),
        fold(num, add, app, lam, vari, let, body))
  }

  def numOfExp(exp: Exp): Int = fold[Int](_ => 1, (l, r) => 1+ l + r, (l, r) => 1 + l + r, (_, _, b) => 1+ b, _ => 1, (_, b1, b2) => 1 + b1 + b2, exp)
}