...
class ParserTest extends AnyFunSuite {
  ...
  test("Expression test") {
    ...
    testSuccess(parser.exp(_))(
      "let x: Any = y in x",
      Let(Seq(Name("x")), Some(TAny), Var("y"), Var("x"))
    )
    testSuccess(parser.exp(_))(
      "let (x, y) = tuple in (y, x)",
      Let(Seq(Name("x"), Name("y")), None, Var("tuple"), Tuple(Seq(Var("y"), Var("x"))))
    )

    val letExp = Let(
      Seq(Name("x"), Name("y")),
      Some(TTuple(Seq(TAny, TNothing))),
      Var("tuple"),
      Tuple(Seq(Var("y"), Var("x")))
    )
    ...
    testSuccess(parser.baseApplyInfixExp(_))(
      "x + y",
      BaseApplyInfix(Var("x"), Scala(meta.Term.Name("+")), Var("y"))
    )
