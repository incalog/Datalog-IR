...
class GenerateDatalogTest extends AnyFunSuite {
          ...
          List(
            Assert(
              Eq(
                PathAccess(
                  Cast(
                    PathAccess(Cast(PathAccess(Var("add"), lhsLink), addType), lhsLink),
                    addType
                  ),
                  lhsLink
                ),
      ...
      List(
        Body(
          List(
            Yield(
              PathAccess(
                Cast(PathAccess(Cast(PathAccess(Var("add"), lhsLink), addType), lhsLink), addType),
                lhsLink
              )
            )
          )
  ...
  test("function pattern calling other function") {
    ...
    val fun = PatternFunction(
      ...
      List(
        Body(
          List(Assign(Seq("lhschild"), Call("lhChild", Seq(Var("add")))), Yield(Var("lhschild")))
        )
      )
    )
    val result =
