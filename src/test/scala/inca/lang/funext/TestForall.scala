...
class TestForall extends AnyFlatSpec with IncaMatchers {
  ...
  "desugaring" should "implement forall list semantics" in {
    val module = Module("Test_Cast", Seq(), Seq(
      PatternFunction(None, "intLists", Seq(Param("l", Some(TList(TNode(Exp.expTag))))), Seq(), Seq(Body(Seq(
        Forall("e", Var("l").typed(TList(TNode(Exp.expTag))), Seq(
    ...
    assertMatch(module, "forallCond", input, scope, Forall) { matcher =>
      assert(matcher.getAllMatchArrays.size == 3)
    }

    assertMatch(module, "intLists", input, scope, Forall) { matcher =>
