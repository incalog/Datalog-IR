...
import inca.viatra.runtime.index.DynamicKey
import inca.viatra.runtime.index.binary.BidirectionalManyToOneIndex
...
class ParentIndex extends BidirectionalManyToOneIndex[URI, URI](ParentIndex.Key)
  ...
  override def processEdit(edit: truechange.CoreEdit): Unit = edit match {
    ...
    case truechange.Attach(node, _, link, parent, _) => link.getRawLink match {
      ...
      case _: ListNextLink =>
        Option(index.get(parent)) match {
          case Some(containingList) => database.iterateNext(node)(insertOrUpdate(_, containingList))
          case None =>
        }
      case RootLink => // skip
    ...
    case truechange.Detach(node, _, link, parent, _) => link.getRawLink match {
      ...
      case _: ListNextLink =>
        Option(index.get(parent)) match {
          case Some(containingList) => database.iterateNext(node)(deletions.put(_, containingList))
          case None =>
        }
      case RootLink => // skip
