...
import inca.ir.extension.{mono, disjunction}
...
import inca.ir.extension.{aggregate, block, demand, impure, mono, arithmetic as incaArithmetic, bool as incaBool, data as incaData, set as incaSet, string as incaString, tuple as incaTuple}
...
case class CompiledSetMonoOptModule(mod: Module) extends CompiledModule:
  ...
  setPipeline(List(
    ...
    () => new demandLowering {}
    /* TODO:
        1. show error messages when there are demand parameters while generating VIATRA code
        2. clarify that demand lowering might not work correctly if
           there exists disjunctions or blocks in the program
    */
  ))
