...
case class CaseDefinitionImport(name: Name, args: Seq[Type], data: TData) extends ModuleImport with CaseDefinitionGeneral:
...
case class TData(ref: Ref[DataDefinitionGeneral]) extends Type:
...
case class Construct(caseRef: Ref[CaseDefinitionGeneral], args: Seq[Term]) extends Term:
...
case class Deconstruct(t: Term, caseRef: Ref[CaseDefinitionGeneral], args: Seq[Arg], neg: Boolean) extends Atom:
