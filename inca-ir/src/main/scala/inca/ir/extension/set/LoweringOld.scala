...
import inca.ir.extension.set.SetLit
...
trait LoweringOld extends BaseLowering:
  ...
  private def refunctionalizeTerm(term: Term): Seq[Term] = refunctionalize() {
    term match
      ...
      case SetLit(Seq()) => Seq(this.emptySet)
      case SetLit(ts) => ts.flatMap(visitTerm)
      ...
      case SetIntersection(t1, t2) =>
        ...
        val baseCase = refunctionalizeTerm(SetLit.empty)
  ...
  private def defunctionalizeTerm(term: Term): Seq[Term] = term match {
    ...
    case SetLit(_) | SetUnion(_, _) | SetIntersection(_, _) =>
