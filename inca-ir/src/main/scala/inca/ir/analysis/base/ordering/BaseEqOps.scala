...
trait BaseEqOps extends EqOps[Value, Topped[Boolean]]:
  override def equ(v1: Value, v2: Value): Topped[Boolean] = (v1, v2) match
    case (Value.Bottom, Value.Bottom) => Topped.Actual(true)
    case (Value.Bottom, _) | (_, Value.Bottom) => Topped.Actual(false)
    case _ => Topped.Top

  override def neq(v1: Value, v2: Value): Topped[Boolean] = (v1, v2) match
    case (Value.Bottom, Value.Bottom) => Topped.Actual(false)
    case (Value.Bottom, _) | (_, Value.Bottom) => Topped.Actual(true)
    case _ => Topped.Top
