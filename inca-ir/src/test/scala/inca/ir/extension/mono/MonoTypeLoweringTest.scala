...
class MonoTypeLoweringTest extends AnyFunSuiteLike {
  ...
  private lazy val relation1: Relation = Relation(
    ...
    Seq(Body(Seq(
      ...
      Impure(RefByName("counter"), Seq(), Var("counter"), MonoImpurityKind),
  ...
  private lazy val relationCastType: Relation = Relation(
    "main",
    Seq(
      Param("b", TInt)
    ),
    Seq(Body(Seq(
      Eq(Var("m"), NewMono(CountFrom, Seq(TString), Seq(IntNum(5)))),
      WriteMono(Var("m"), Cast(IntNum(1), TAny), Seq(StringLit("A"))),
      Eq(Var("b"), ReadMono(Var("m")))
    ))))

  test("Lower Mono Types: test subtypes") {
    implicit val typechecker1 = new IRTypechecker {}
