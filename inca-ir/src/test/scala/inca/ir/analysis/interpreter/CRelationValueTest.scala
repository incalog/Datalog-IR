...
import inca.ir.analysis.base.values.CRelationValue
import org.scalatest.funsuite.AnyFunSuiteLike

class CRelationValueTest extends AnyFunSuiteLike:
  ...
  test("Natural Join - Table x Table (single shared column)") {
    val table1 = CRelationValue(Seq("a", "b"), Set(Seq(1, 2), Seq(3, 4)))
    val table2 = CRelationValue(Seq("b", "c"), Set(Seq(2, 5), Seq(4, 6)))
    val result = table1.naturalJoin(table2)
    val expected = CRelationValue(Seq("a", "b", "c"), Set(Seq(1, 2, 5), Seq(3, 4, 6)))
  ...
  test("Natural Join - Table x Table (no shared common columns)") {
    ...
    val result = table1.naturalJoin(table2)
    val expected = CRelationValue(Seq("a", "b", "c", "d"), Set(Seq(1, 2, 5, 6), Seq(1, 2, 7, 8), Seq(3, 4, 5, 6), Seq(3, 4, 7, 8)))
    assert(result == expected)
  }

  test("Natural Join - Table x Table (multiple shared columns 1)") {
    val table1 = CRelationValue(Seq("a", "b", "c"), Set(Seq(1, 2, 3), Seq(4, 5, 6)))
    val table2 = CRelationValue(Seq("b", "c", "d"), Set(Seq(2, 3, 4), Seq(5, 6, 7)))
    val result = table1.naturalJoin(table2)
    val expected = CRelationValue(Seq("a", "b", "c", "d"), Set(Seq(1, 2, 3, 4), Seq(4, 5, 6, 7)))
    assert(result == expected)
  }

  test("Natural Join - Table x Table (multiple shared columns 2)") {
    val table1 = CRelationValue(Seq("a", "b", "c"), Set(Seq(1, 2, 3), Seq(4, 5, -1)))
    val table2 = CRelationValue(Seq("b", "c", "d"), Set(Seq(2, 3, 4), Seq(5, 6, 7)))
    val result = table1.naturalJoin(table2)
    val expected = CRelationValue(Seq("a", "b", "c", "d"), Set(Seq(1, 2, 3, 4)))
    assert(result == expected)
  }
  ...
  test("Union - Table x Table (same columns )") {
    val table1 = CRelationValue(Seq("a", "b"), Set(Seq(1, 2), Seq(3, 4)))
    val table2 = CRelationValue(Seq("a", "b"), Set(Seq(5, 6), Seq(7, 8)))
    val result = table1.union(table2)
    val expected = CRelationValue(Seq("a", "b"), Set(Seq(1, 2), Seq(3, 4), Seq(5, 6), Seq(7, 8)))
    assert(result == expected)
  }

  test("Union - Table x Table (same columns rearranged)") {
    val table1 = CRelationValue(Seq("a", "b"), Set(Seq(1, 2), Seq(3, 4)))
    val table2 = CRelationValue(Seq("b", "a"), Set(Seq(5, 6), Seq(7, 8)))
    val result = table1.union(table2)
    val expected = CRelationValue(Seq("a", "b"), Set(Seq(1, 2), Seq(3, 4), Seq(6, 5), Seq(8, 7)))
    assert(result == expected)
  }

  test("Union - Table x Table (different columns | different size)") {
    val table1 = CRelationValue(Seq("a", "b", "c"), Set(Seq(1, 2, 3), Seq(4, 5, 6)))
    val table2 = CRelationValue(Seq("c", "d"), Set(Seq(5, 6), Seq(7, 8), Seq(9, 10)))
  ...
  test("Union - Table x Table (different columns | same size)") {
    val table1 = CRelationValue(Seq("a", "b"), Set(Seq(1, 2), Seq(4, 5)))
    val table2 = CRelationValue(Seq("c", "d"), Set(Seq(5, 6), Seq(7, 8), Seq(9, 10)))
    assertThrows[IllegalArgumentException] {
      table1.union(table2)
    }
  ...
  test("Cartesian - Table x Table (shared columns)") {
    val table1 = CRelationValue(Seq("a", "b"), Set(Seq(1, 2), Seq(3, 4)))
    val table2 = CRelationValue(Seq("b", "c"), Set(Seq(5, 6), Seq(7, 8)))
    assertThrows[IllegalArgumentException] {
